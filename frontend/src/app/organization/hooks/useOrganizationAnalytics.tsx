...
export default function useOrganizationAnalytics(
  trigger = true
) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const pres = await fetch(`${ORG1_ANALYTICS}/presence?days=35`, { credentials: "include" });
        if (pres.ok) {
        ...
        const org = await fetch(`${ORG2_ANALYTICS}/posts?weeks=8`, { credentials: "include" });
        if (org.ok) {
